...
By default the results for each run are saved in two reports: _ChartMediaDiffLink_RUN_x.png_ indicates the rmessages difference for each iteration, _factor_graph_run_x.txt_ provides a textual description of the factor graph (x represents the number of instance). Moreover, after each run the program creates two graphs that plot the average message difference and the number of conflicts (y axis) against the iterations (x axis). 
